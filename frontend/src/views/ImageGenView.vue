  ...
  >
    ...
    <div v-if="isDragging" class="drag-overlay">
      <div class="drag-hint">
        ...
        <span>拖放图片以创建图生图任务</span>
      </div>
    </div>

    <!-- 顶部标题栏 -->
    <header class="view-header">
      <h1 class="view-title">图片生成</h1>
    ...
    <div class="task-list-area">
        ...
        >
          ...
          <div class="task-info">
            ...
            <div class="task-meta">
              ...
              <span class="task-mode-badge">{{ task.mode === 'img2img' ? '图生图' : '文生图' }}</span>
          ...
          <div v-if="task.status === 'done' && task.result_image" class="task-thumb">
          ...
          <div class="task-actions" @click.stop>
            ...
            <button v-if="task.status === 'done'" class="action-btn" title="保存" @click="saveTaskImage(task.id)">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
      ...
      <div v-else class="empty-state">
        ...
        <p class="empty-text">点击右上角添加任务，或拖拽图片进入</p>
        <p class="empty-hint">支持多任务并发，拖拽多张图片批量创建图生图任务</p>
      </div>
    </div>
    ...
    <div v-if="expandedTask && expandedTask.status === 'done'" class="preview-overlay" @click="expandedId = null">
      <div class="preview-container" @click.stop>
        <img :src="getImageSrc(expandedTask)" class="preview-image" />
        <div class="preview-actions">
          <button class="btn btn-secondary" @click="saveTaskImage(expandedTask.id)">保存图片</button>
    ...
    <div v-if="showTextDialog" class="dialog-overlay" @click.self="showTextDialog = false">
      <div class="dialog">
        <div class="dialog-header">
          <h2 class="dialog-title">添加文生图任务</h2>
        ...
        <div class="dialog-body">
          ...
          <div class="custom-select" @click.stop>
            ...
            <div v-if="textModelOpen" class="custom-select-dropdown">
              <div class="select-group-label">标准</div>
              <button v-for="m in MODELS.standard" :key="m.value" class="select-option" :class="{ selected: textModel === m.value }" @click="textModel = m.value; textModelOpen = false">{{ m.label }}</button>
              <div class="select-group-label">2K 高清</div>
              <button v-for="m in MODELS.hd2k" :key="m.value" class="select-option" :class="{ selected: textModel === m.value }" @click="textModel = m.value; textModelOpen = false">{{ m.label }}</button>
              <div class="select-group-label">4K 超清</div>
              <button v-for="m in MODELS.hd4k" :key="m.value" class="select-option" :class="{ selected: textModel === m.value }" @click="textModel = m.value; textModelOpen = false">{{ m.label }}</button>
            </div>
          </div>
        </div>
    ...
    <div v-if="showImgDialog" class="dialog-overlay" @click.self="showImgDialog = false">
      <div class="dialog dialog-wide">
        <div class="dialog-header">
          <h2 class="dialog-title">添加图生图任务</h2>
            ...
            <div class="img-entry-fields">
              ...
              <div class="custom-select custom-select-sm" @click.stop>
                ...
                <div v-if="img.modelOpen" class="custom-select-dropdown">
                  <div class="select-group-label">标准</div>
                  <button v-for="m in MODELS.standard" :key="m.value" class="select-option" :class="{ selected: img.model === m.value }" @click="img.model = m.value; img.modelOpen = false">{{ m.label }}</button>
                  <div class="select-group-label">2K 高清</div>
                  <button v-for="m in MODELS.hd2k" :key="m.value" class="select-option" :class="{ selected: img.model === m.value }" @click="img.model = m.value; img.modelOpen = false">{{ m.label }}</button>
                  <div class="select-group-label">4K 超清</div>
                  <button v-for="m in MODELS.hd4k" :key="m.value" class="select-option" :class="{ selected: img.model === m.value }" @click="img.model = m.value; img.modelOpen = false">{{ m.label }}</button>
                </div>
              </div>
            </div>
...
const textModel = ref('gemini-3.0-pro-image-landscape')
...
const MODELS = {
  standard: [
    { value: 'gemini-3.0-pro-image-landscape', label: '横屏' },
    { value: 'gemini-3.0-pro-image-portrait', label: '竖屏' },
  ],
  hd2k: [
    { value: 'gemini-3.0-pro-image-landscape-2k', label: '横屏 2K' },
    { value: 'gemini-3.0-pro-image-portrait-2k', label: '竖屏 2K' },
  ],
  hd4k: [
    { value: 'gemini-3.0-pro-image-landscape-4k', label: '横屏 4K' },
    { value: 'gemini-3.0-pro-image-portrait-4k', label: '竖屏 4K' },
  ],
}

const MODEL_LABELS = {}
;[...MODELS.standard, ...MODELS.hd2k, ...MODELS.hd4k].forEach(m => MODEL_LABELS[m.value] = m.label)

function formatModel(model) { return MODEL_LABELS[model] || model }

function getImageSrc(task) {
  if (task.result_image_type === 'url') return task.result_image
  return 'data:image/jpeg;base64,' + task.result_image
}
...
async function submitTextTasks() {
  ...
  for (const line of lines) {
    try {
      const task = await window.pywebview.api.add_image_task(line, textModel.value, 'text2img', '')
  ...
  toast.success(`已添加 ${count} 个文生图任务`)
  textPrompts.value = ''
...
async function submitImgTasks() {
  ...
  for (const entry of imgEntries.value) {
    try {
      const task = await window.pywebview.api.add_image_task(entry.prompt, entry.model, 'img2img', entry.base64)
      tasks.value.unshift(task)
      count++
    } catch (e) {
  ...
  toast.success(`已添加 ${count} 个图生图任务`)
  imgEntries.value = []
...
async function cancelTask(id) {
  try {
    await window.pywebview.api.cancel_task(id)
...
async function clearDoneTasks() {
  try {
    await window.pywebview.api.clear_done_tasks()
...
async function saveTaskImage(id) {
  try {
    const path = await window.pywebview.api.save_task_image(id)
    if (path) toast.success('图片已保存')
...
function onTaskUpdate(data) {
  ...
  else if (data.type === 'done') { task.status = 'done'; task.result_image = data.result_image; task.result_image_type = data.result_image_type; task.file_path = data.file_path || '' }
  else if (data.type === 'error') { task.status = 'error'; task.error = data.error }
  else if (data.type === 'cancelled') task.status = 'cancelled'
}

onMounted(async () => {
  window.__onTaskUpdate = onTaskUpdate
  // 点击任何地方关闭下拉框
  document.addEventListener('click', closeAllDropdowns)
  try {
    const list = await window.pywebview.api.get_all_tasks()
    if (list && list.length) tasks.value = list
  } catch (e) { /* ignore */ }
})

onUnmounted(() => {
  delete window.__onTaskUpdate
...
.task-thumb-img { width: 40px; height: 40px; border-radius: var(--radius-sm); object-fit: cover; border: 1px solid var(--border); }
...
.preview-image { max-width: 100%; max-height: calc(90vh - 60px); border-radius: var(--radius-md); object-fit: contain; }
